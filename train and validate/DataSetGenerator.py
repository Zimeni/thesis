...
class DataSetGenerator:
    ...
    def get_mini_batches(self, batch_size=10, image_size=(252, 252), allchannel=True):
        images = []
        labels = []
        empty=False
        counter=0
        ...
        while True:
            for i in range(len(self.data_labels)):
                ...
                print("IMAGE FILENAME: " + str(self.data_info[i][counter]))
                img = self.resizeAndPad(img, image_size)

                #else:
                    #print("IMAGE FILENAME: " + str(img.filename))
                    #continue
                if not allchannel:
